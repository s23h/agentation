<script lang="ts">
  ...
  } from '../utils/design-mode/types';
  import { detectPageSections } from '../utils/design-mode/section-detection';
  import { generateDesignOutput, generateRearrangeOutput } from '../utils/design-mode/output';
  ...
  function generateOutput(
    ...
    // Append layout mode output if active
    if (isLayoutMode) {
      const layoutOut = generateLayoutOutput();
      if (layoutOut) output += '\n\n' + layoutOut;
    }
  ...
  // =========================================================================
  // Layout Mode State (AFS 1.1)
  // =========================================================================

  let isLayoutMode = $state(false);
  let layoutModeExiting = $state(false);
  let isWireframeMode = $state(false);
  ...
  let isDraggingPlacement = $state(false);
  let isDraggingSection = $state(false);
  let layoutPaletteSection = $state<string | null>(null);

  // Stash state when toggling wireframe mode (preserves explore vs wireframe state)
  let exploreStash = { rearrange: null as RearrangeState | null, placements: [] as DesignPlacement[] };
  let wireframeStash = { rearrange: null as RearrangeState | null, placements: [] as DesignPlacement[] };

  // Drag refs for placement / section dragging
  let placementDragRef: {
    id: string;
    startMouseX: number;
    startMouseY: number;
    startX: number;
  ...
  let layoutModeExitTimerRef: ReturnType<typeof setTimeout> | null = null;
  ...
  function enterLayoutMode() {
    ...
    showSettings = false;
    isLayoutMode = true;
    layoutModeExiting = false;
    layoutPaletteSection = COMPONENT_REGISTRY[0].section;
    // Detect page sections for rearrange mode if not already done
    if (!rearrangeState) {
  ...
  function closeLayoutMode() {
    layoutModeExiting = true;
    isLayoutMode = false;
    activeDesignComponent = null;
    selectedPlacementId = null;
    selectedSectionId = null;
    if (layoutModeExitTimerRef) clearTimeout(layoutModeExitTimerRef);
    layoutModeExitTimerRef = setTimeout(() => {
      layoutModeExiting = false;
    }, 300);
  }

  function toggleLayoutMode() {
    if (isLayoutMode) closeLayoutMode();
    else enterLayoutMode();
  }

  function toggleWireframeMode() {
    if (isWireframeMode) {
      // Switching back to explore: save wireframe stash, restore explore stash
      ...
      // Switching to wireframe: save explore stash, restore wireframe stash
      exploreStash = { rearrange: rearrangeState, placements: designPlacements };
      rearrangeState = wireframeStash.rearrange;
  ...
  function addPlacement(e: MouseEvent) {
    if (!activeDesignComponent) return;
    const scrollY = window.scrollY;
    const sizes = DEFAULT_SIZES[activeDesignComponent];
    const x = e.clientX - sizes.width / 2;
    const y = e.clientY + scrollY - sizes.height / 2;

    const placement: DesignPlacement = {
      id: `dp-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      type: activeDesignComponent,
      x,
      y,
      width: sizes.width,
      height: sizes.height,
  ...
  function deletePlacement(id: string) {
    designPlacements = designPlacements.filter((p) => p.id !== id);
  ...
  function clearLayoutMode() {
    ...
    selectedSectionId = null;
    if (rearrangeState) {
      // Reset all sections to original rects
      rearrangeState = {
        ...rearrangeState,
        sections: rearrangeState.sections.map((s) => ({
          ...s,
          currentRect: { ...s.originalRect },
        })),
      };
    }
  }

  function startPlacementDrag(e: MouseEvent, placement: DesignPlacement) {
    e.stopPropagation();
    selectedPlacementId = placement.id;
  ...
  function startSectionDrag(e: MouseEvent, section: DetectedSection) {
    e.stopPropagation();
    ...
    sectionDragRef = {
      ...
      startMouseX: e.clientX,
      startMouseY: e.clientY,
      startX: section.currentRect.x,
      startY: section.currentRect.y,
    };
  }

  function startPlacementResize(e: MouseEvent, placement: DesignPlacement) {
    e.stopPropagation();
    selectedPlacementId = placement.id;
    placementResizeRef = {
      id: placement.id,
  ...
  function handleLayoutMouseMove(e: MouseEvent) {
    if (placementDragRef) {
      const dx = e.clientX - placementDragRef.startMouseX;
      const dy = e.clientY - placementDragRef.startMouseY;
      designPlacements = designPlacements.map((p) =>
        p.id === placementDragRef!.id
          ? { ...p, x: placementDragRef!.startX + dx, y: placementDragRef!.startY + dy }
          : p,
      );
    }
    if (sectionDragRef && rearrangeState) {
      const dx = e.clientX - sectionDragRef.startMouseX;
      const dy = e.clientY - sectionDragRef.startMouseY;
      rearrangeState = {
        ...rearrangeState,
        sections: rearrangeState.sections.map((s) =>
          s.id === sectionDragRef!.id
            ? {
                ...s,
                currentRect: {
                  ...s.currentRect,
                  x: sectionDragRef!.startX + dx,
                  y: sectionDragRef!.startY + dy,
                },
              }
            : s,
        ),
    ...
    if (placementResizeRef) {
      const dx = e.clientX - placementResizeRef.startMouseX;
      const dy = e.clientY - placementResizeRef.startMouseY;
      designPlacements = designPlacements.map((p) =>
        p.id === placementResizeRef!.id
          ? {
              ...p,
              width: Math.max(20, placementResizeRef!.startW + dx),
              height: Math.max(10, placementResizeRef!.startH + dy),
            }
          : p,
      );
    }
  ...
  function handleLayoutMouseUp() {
    placementDragRef = null;
    sectionDragRef = null;
    placementResizeRef = null;
    isDraggingPlacement = false;
    isDraggingSection = false;
  }

  function generateLayoutOutput(): string {
    const viewport = { width: window.innerWidth, height: window.innerHeight };
    const detailLevel = settings.outputDetail;
    const parts: string[] = [];

    if (designPlacements.length > 0) {
      const placementOut = generateDesignOutput(
        designPlacements,
        viewport,
        isWireframeMode ? { blankCanvas: true, wireframePurpose: wireframePurpose || undefined } : undefined,
        detailLevel,
      );
      if (placementOut) parts.push(placementOut);
    }

    if (rearrangeState) {
      const rearrangeOut = generateRearrangeOutput(rearrangeState, detailLevel, viewport);
      if (rearrangeOut) parts.push(rearrangeOut);
    }
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDraggingToolbar || distance > TOOLBAR_DRAG_THRESHOLD) {
        ...
        const contentWidth = isActive
          ? connectionStatus === 'connected' ? 337 : 297
          : 44;
...
{#if mounted && !isToolbarHidden}
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          ...
          <div class="buttonWrapper">
            <button class="controlButton" onclick={(e: MouseEvent) => { e.stopPropagation(); hideTooltipsUntilMouseLeave(); toggleLayoutMode(); }} data-active={isLayoutMode}>
            ...
            <span class="buttonTooltip">Layout mode<span class="shortcut">L</span></span>
          </div>
    ...
    <!-- Layout Mode Overlay -->
    {#if isLayoutMode || layoutModeExiting}
      <!-- Canvas dimmer for wireframe mode -->
      {#if isWireframeMode}
        <div
          class="layoutCanvasDimmer"
          data-feedback-toolbar
          style="opacity: {1 - canvasOpacity};"
        ></div>
      {/if}

      <!-- Placed component overlays -->
      {#each designPlacements as placement (placement.id)}
        {@const isSelected = selectedPlacementId === placement.id}
        {@const label = COMPONENT_MAP[placement.type]?.label || placement.type}
        <div
          class="layoutPlacement {isSelected ? 'selected' : ''}"
          data-feedback-toolbar
          style="left: {placement.x}px; top: {placement.y - scrollY}px; width: {placement.width}px; height: {placement.height}px;"
          onmousedown={(e: MouseEvent) => startPlacementDrag(e, placement)}
          onclick={(e: MouseEvent) => { e.stopPropagation(); selectedPlacementId = placement.id; }}
        >
          <span class="layoutPlacementLabel">{label}</span>
          <span class="layoutPlacementSize">{Math.round(placement.width)}×{Math.round(placement.height)}</span>
          {#if isSelected}
            <button
              class="layoutPlacementDelete"
              onmousedown={(e: MouseEvent) => e.stopPropagation()}
              onclick={(e: MouseEvent) => { e.stopPropagation(); deletePlacement(placement.id); }}
              title="Delete"
            >×</button>
            <div
              class="layoutPlacementResizeHandle"
              onmousedown={(e: MouseEvent) => startPlacementResize(e, placement)}
            ></div>
          {/if}
        </div>
      {/each}

      <!-- Rearrange section overlays -->
      {#if rearrangeState}
        {#each rearrangeState.sections as section (section.id)}
          {@const isSelected = selectedSectionId === section.id}
          {@const isHovered = hoveredSectionId === section.id}
          {@const hasMoved = Math.abs(section.currentRect.x - section.originalRect.x) > 1 || Math.abs(section.currentRect.y - section.originalRect.y) > 1}
          <div
            class="layoutSection {isSelected ? 'selected' : ''} {isHovered ? 'hovered' : ''} {hasMoved ? 'moved' : ''}"
            data-feedback-toolbar
            style="left: {section.currentRect.x}px; top: {section.currentRect.y - scrollY}px; width: {section.currentRect.width}px; height: {section.currentRect.height}px;"
            onmouseenter={() => (hoveredSectionId = section.id)}
            onmouseleave={() => (hoveredSectionId = null)}
            onmousedown={(e: MouseEvent) => startSectionDrag(e, section)}
            onclick={(e: MouseEvent) => { e.stopPropagation(); selectedSectionId = section.id; }}
          >
            <span class="layoutSectionLabel">{section.label}</span>
            {#if hasMoved}
              <span class="layoutSectionMoved">↑ moved</span>
            {/if}
          </div>
        {/each}
      {/if}

      <!-- Drag cursor overlay when component selected -->
      {#if activeDesignComponent}
        <div
          class="layoutDropCursor"
          data-feedback-toolbar
          onclick={(e: MouseEvent) => { e.stopPropagation(); addPlacement(e); }}
        >
          {@const sizes = DEFAULT_SIZES[activeDesignComponent]}
          {@const label = COMPONENT_MAP[activeDesignComponent]?.label || activeDesignComponent}
          <div class="layoutDropPreview" style="width: {sizes.width}px; height: {sizes.height}px;">
            <span>{label}</span>
            <span class="layoutPlacementSize">{sizes.width}×{sizes.height}</span>
          </div>
        </div>
      {/if}
      ...
      <div
        class="layoutDragCapture"
        data-feedback-toolbar
        class:active={isDraggingPlacement || isDraggingSection || !!placementResizeRef}
        onmousemove={handleLayoutMouseMove}
        onmouseup={handleLayoutMouseUp}
        onmouseleave={handleLayoutMouseUp}
      ></div>

      <!-- Layout Mode Panel (palette + controls) -->
      <div
        class="layoutPanel {layoutModeExiting ? 'exit' : 'enter'}"
        data-feedback-toolbar
        onclick={(e: MouseEvent) => e.stopPropagation()}
      >
        <div class="layoutPanelHeader">
          <span class="layoutPanelTitle">Layout Mode</span>
          <div class="layoutPanelHeaderRight">
            {#if designPlacements.length > 0 || (rearrangeState?.sections.some(s => Math.abs(s.currentRect.x - s.originalRect.x) > 1 || Math.abs(s.currentRect.y - s.originalRect.y) > 1))}
              <button class="layoutClearBtn" onclick={clearLayoutMode} title="Clear all placements and reset rearrangements">
                Clear
              </button>
            {/if}
            <button class="layoutCloseBtn" onclick={closeLayoutMode} title="Exit layout mode (L)">×</button>
          </div>
        </div>

        <!-- Wireframe toggle -->
        <div class="layoutWireframeRow">
          <label class="layoutWireframeToggle">
            <input
              type="checkbox"
              checked={isWireframeMode}
              onchange={toggleWireframeMode}
            />
            <span class="layoutWireframeCheck {isWireframeMode ? 'checked' : ''}"></span>
            <span class="layoutWireframeLabel">Wireframe New Page</span>
          </label>
          {#if isWireframeMode}
            <div class="layoutOpacityRow">
              <span class="layoutOpacityLabel">Opacity</span>
              <input
                type="range"
                min="0"
                max="1"
                step="0.05"
                value={canvasOpacity}
                oninput={(e: Event) => (canvasOpacity = parseFloat((e.target as HTMLInputElement).value))}
                class="layoutOpacitySlider"
              />
            </div>
            <input
              type="text"
              class="layoutPurposeInput"
              placeholder="Purpose (e.g. landing page with pricing table)"
              value={wireframePurpose}
              oninput={(e: Event) => (wireframePurpose = (e.target as HTMLInputElement).value)}
        ...
        <!-- Component Palette -->
        <div class="layoutPaletteContainer">
          <div class="layoutPaletteSections">
            {#each COMPONENT_REGISTRY as section}
              <button
                class="layoutPaletteSectionTab {layoutPaletteSection === section.section ? 'active' : ''}"
                onclick={() => (layoutPaletteSection = section.section)}
              >
                {section.section}
              </button>
            {/each}
          </div>
          <div class="layoutPaletteGrid">
            {#each COMPONENT_REGISTRY.find(s => s.section === layoutPaletteSection)?.items ?? [] as item}
              <button
                class="layoutPaletteItem {activeDesignComponent === item.type ? 'active' : ''}"
                onclick={() => {
                  activeDesignComponent = activeDesignComponent === item.type ? null : item.type;
                  selectedPlacementId = null;
                }}
        ...
        <div class="layoutPanelFooter">
          {#if designPlacements.length > 0}
            <span class="layoutCount">{designPlacements.length} placed</span>
          {/if}
          {#if rearrangeState?.sections.some(s => Math.abs(s.currentRect.x - s.originalRect.x) > 1 || Math.abs(s.currentRect.y - s.originalRect.y) > 1)}
            <span class="layoutCount">
              {rearrangeState?.sections.filter(s => Math.abs(s.currentRect.x - s.originalRect.x) > 1 || Math.abs(s.currentRect.y - s.originalRect.y) > 1).length} moved
            </span>
          {/if}
          <span class="layoutHint">
            {#if activeDesignComponent}
              Click to place · Esc to cancel
            {:else}
              Click palette to place · Drag section to rearrange
            {/if}
          </span>
        </div>
      </div>
...
<style>
  ...
  .toolbar { position: fixed; width: 337px; z-index: 100000; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; pointer-events: none; transition: left 0s, top 0s, right 0s, bottom 0s; }
  ...
  .layoutCanvasDimmer { position: fixed; inset: 0; background: #000; pointer-events: none; z-index: 99990; transition: opacity 0.2s; }

  /* Placed component boxes */
  .layoutPlacement {
    position: absolute;
    border: 1.5px dashed rgba(120, 120, 255, 0.6);
  ...
  .layoutPlacement.selected {
    border-color: rgba(100, 100, 255, 0.9);
    background: rgba(100, 100, 255, 0.1);
  }
  .layoutPlacement:hover { border-color: rgba(120, 120, 255, 0.85); }
  .layoutPlacementLabel { font-size: 11px; font-weight: 500; color: rgba(120, 120, 255, 0.9); pointer-events: none; user-select: none; text-align: center; padding: 0 4px; }
  .layoutPlacementSize { font-size: 10px; color: rgba(120, 120, 255, 0.5); pointer-events: none; user-select: none; }
  .layoutPlacementDelete { position: absolute; top: 2px; right: 4px; background: none; border: none; color: rgba(255,80,80,0.7); cursor: pointer; font-size: 14px; line-height: 1; padding: 0 2px; z-index: 1; }
  .layoutPlacementDelete:hover { color: rgba(255,80,80,1); }
  .layoutPlacementResizeHandle { position: absolute; bottom: 0; right: 0; width: 10px; height: 10px; cursor: se-resize; border-right: 2px solid rgba(120,120,255,0.7); border-bottom: 2px solid rgba(120,120,255,0.7); border-radius: 0 0 3px 0; }
  ...
  .layoutSection.hovered { border-color: rgba(80,200,120,0.5); background: rgba(80,200,120,0.04); }
  .layoutSection.selected { border-color: rgba(80,200,120,0.8); background: rgba(80,200,120,0.07); }
  .layoutSection.moved { border-color: rgba(255,180,0,0.6); background: rgba(255,180,0,0.05); }
  .layoutSection.moved.selected { border-color: rgba(255,180,0,0.9); }
  .layoutSectionLabel { font-size: 10px; font-weight: 500; color: rgba(80,200,120,0.85); pointer-events: none; user-select: none; background: rgba(0,0,0,0.45); padding: 1px 5px; border-radius: 3px; }
  .layoutSectionMoved { font-size: 9px; color: rgba(255,180,0,0.8); pointer-events: none; user-select: none; background: rgba(0,0,0,0.45); padding: 1px 5px; border-radius: 3px; }

  /* Drop cursor overlay */
  .layoutDropCursor { position: fixed; inset: 0; cursor: crosshair; z-index: 99995; pointer-events: auto; display: flex; align-items: center; justify-content: center; }
  .layoutDropPreview { border: 1.5px dashed rgba(120,120,255,0.7); background: rgba(100,100,255,0.08); border-radius: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; pointer-events: none; }
  .layoutDropPreview span { font-size: 12px; font-weight: 500; color: rgba(120,120,255,0.9); }

  /* Drag capture layer */
  .layoutDragCapture { position: fixed; inset: 0; z-index: 99994; pointer-events: none; }
  .layoutDragCapture.active { pointer-events: auto; cursor: grabbing; }
  ...
  .layoutPanel.enter { animation: scaleIn 0.2s cubic-bezier(0.19,1,0.22,1) forwards; }
  .layoutPanel.exit { animation: fadeOut 0.2s ease forwards; pointer-events: none; }

  .layoutPanelHeader { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px 8px; border-bottom: 1px solid rgba(255,255,255,0.08); }
  .layoutPanelTitle { font-size: 12px; font-weight: 600; color: rgba(255,255,255,0.85); }
  .layoutPanelHeaderRight { display: flex; align-items: center; gap: 6px; }
  .layoutClearBtn { background: none; border: none; color: rgba(255,100,100,0.7); font-size: 11px; cursor: pointer; padding: 2px 4px; border-radius: 4px; }
  .layoutClearBtn:hover { color: rgba(255,100,100,1); background: rgba(255,100,100,0.1); }
  .layoutCloseBtn { background: none; border: none; color: rgba(255,255,255,0.4); font-size: 18px; cursor: pointer; line-height: 1; padding: 0 2px; }
  .layoutCloseBtn:hover { color: rgba(255,255,255,0.8); }

  /* Wireframe controls */
  .layoutWireframeRow { padding: 8px 12px; border-bottom: 1px solid rgba(255,255,255,0.06); display: flex; flex-direction: column; gap: 6px; }
  .layoutWireframeToggle { display: flex; align-items: center; gap: 7px; cursor: pointer; }
  .layoutWireframeCheck { width: 14px; height: 14px; border: 1.5px solid rgba(255,255,255,0.3); border-radius: 3px; flex-shrink: 0; }
  .layoutWireframeCheck.checked { background: var(--agentation-color-accent); border-color: var(--agentation-color-accent); }
  .layoutWireframeLabel { font-size: 11.5px; color: rgba(255,255,255,0.75); }
  .layoutOpacityRow { display: flex; align-items: center; gap: 8px; margin-top: 2px; }
  .layoutOpacityLabel { font-size: 10px; color: rgba(255,255,255,0.4); flex-shrink: 0; }
  .layoutOpacitySlider { flex: 1; height: 3px; accent-color: var(--agentation-color-accent); cursor: pointer; }
  .layoutPurposeInput { width: 100%; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); border-radius: 6px; color: rgba(255,255,255,0.8); font-size: 11px; padding: 5px 8px; box-sizing: border-box; }
  .layoutPurposeInput::placeholder { color: rgba(255,255,255,0.25); }
  .layoutPurposeInput:focus { outline: none; border-color: rgba(255,255,255,0.25); }

  /* Component palette */
  .layoutPaletteContainer { display: flex; flex-direction: column; }
  .layoutPaletteSections { display: flex; padding: 6px 8px 0; gap: 2px; overflow-x: auto; scrollbar-width: none; }
  .layoutPaletteSections::-webkit-scrollbar { display: none; }
  .layoutPaletteSectionTab { background: none; border: none; color: rgba(255,255,255,0.4); font-size: 10px; font-weight: 500; cursor: pointer; padding: 3px 6px; border-radius: 4px; white-space: nowrap; }
  .layoutPaletteSectionTab.active { background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.85); }
  .layoutPaletteSectionTab:hover:not(.active) { color: rgba(255,255,255,0.65); }
  .layoutPaletteGrid { display: flex; flex-wrap: wrap; gap: 4px; padding: 6px 8px 8px; max-height: 130px; overflow-y: auto; scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.15) transparent; }
  .layoutPaletteItem { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 5px; color: rgba(255,255,255,0.7); font-size: 10px; font-weight: 500; cursor: pointer; padding: 4px 7px; white-space: nowrap; }
  .layoutPaletteItem:hover { background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.9); }
  .layoutPaletteItem.active { background: var(--agentation-color-accent); border-color: var(--agentation-color-accent); color: #fff; }

  /* Panel footer */
  .layoutPanelFooter { display: flex; align-items: center; gap: 8px; padding: 6px 12px 8px; border-top: 1px solid rgba(255,255,255,0.06); flex-wrap: wrap; }
  .layoutCount { font-size: 10px; font-weight: 500; color: rgba(120,120,255,0.8); background: rgba(120,120,255,0.12); padding: 2px 6px; border-radius: 10px; }
  .layoutHint { font-size: 10px; color: rgba(255,255,255,0.3); margin-left: auto; text-align: right; }

  /* Light mode overrides for layout mode panel */
  [data-agentation-theme="light"] .layoutPanel { background: #fff; color: #111; box-shadow: 0 4px 20px rgba(0,0,0,0.15), 0 1px 4px rgba(0,0,0,0.08); }
  [data-agentation-theme="light"] .layoutPanelHeader { border-bottom-color: rgba(0,0,0,0.08); }
  [data-agentation-theme="light"] .layoutPanelTitle { color: rgba(0,0,0,0.85); }
  [data-agentation-theme="light"] .layoutWireframeRow { border-bottom-color: rgba(0,0,0,0.06); }
  [data-agentation-theme="light"] .layoutWireframeLabel { color: rgba(0,0,0,0.7); }
  [data-agentation-theme="light"] .layoutWireframeCheck { border-color: rgba(0,0,0,0.25); }
  [data-agentation-theme="light"] .layoutOpacityLabel { color: rgba(0,0,0,0.4); }
  [data-agentation-theme="light"] .layoutPurposeInput { background: rgba(0,0,0,0.04); border-color: rgba(0,0,0,0.12); color: rgba(0,0,0,0.8); }
  [data-agentation-theme="light"] .layoutPurposeInput::placeholder { color: rgba(0,0,0,0.25); }
  [data-agentation-theme="light"] .layoutPaletteSectionTab { color: rgba(0,0,0,0.4); }
  [data-agentation-theme="light"] .layoutPaletteSectionTab.active { background: rgba(0,0,0,0.07); color: rgba(0,0,0,0.85); }
  [data-agentation-theme="light"] .layoutPaletteItem { background: rgba(0,0,0,0.04); border-color: rgba(0,0,0,0.1); color: rgba(0,0,0,0.7); }
  [data-agentation-theme="light"] .layoutPaletteItem:hover { background: rgba(0,0,0,0.08); color: rgba(0,0,0,0.9); }
  [data-agentation-theme="light"] .layoutPanelFooter { border-top-color: rgba(0,0,0,0.06); }
  [data-agentation-theme="light"] .layoutHint { color: rgba(0,0,0,0.3); }
  [data-agentation-theme="light"] .layoutCloseBtn { color: rgba(0,0,0,0.35); }
  [data-agentation-theme="light"] .layoutCloseBtn:hover { color: rgba(0,0,0,0.75); }
  [data-agentation-theme="light"] .layoutSectionLabel { color: rgba(50,160,90,0.9); }
</style>
