...
export function generateRearrangeOutput(
  ...
  for (const { section: s, posMoved, sizeChanged } of changed) {
    ...
    if (detailLevel === 'detailed' || detailLevel === 'forensic') {
      ...
      if (ident !== s.selector) out += `  - Element: \`${ident}\`\n`;
  ...
  if (detailLevel !== 'compact' && sections.length > 1) {
    ...
    for (const s of sortedSections) {
      ...
      const moved =
        Math.abs(r.x - s.originalRect.x) > 1 ||
        Math.abs(r.y - s.originalRect.y) > 1 ||
        Math.abs(r.width - s.originalRect.width) > 1 ||
        Math.abs(r.height - s.originalRect.height) > 1;
